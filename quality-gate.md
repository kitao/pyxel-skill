...
| 8 | Palette hierarchy | `inspect_palette(script="main.py")` returns `verdict in ("pass", "warn")` (which encodes `hierarchy.score == 2`); the underlying `hierarchy.score` field remains available as fallback documentation | asset-planner / asset-gen |
| 9 | Contrast | `inspect_palette(script="main.py")` returns `verdict == "pass"` (which encodes `len(contrast_warnings) <= 1`); the underlying `contrast_warnings` list remains available as fallback documentation | asset-planner / asset-gen |
